...
export async function updateProfileAction(
	data: UpdateProfileSchemaValues,
	...
	try {
		...
		const response = await fetch(
			`${NEXT_PUBLIC_API_URL}/api/v1/auth/profile`,
			{
				method: "PUT",
				headers: {
		...
		const json = (await response
			.json()
			.catch(() => null)) as UpdateProfileActionResponse;

		if (!response.ok) {
			const msg =
				json.message ??
				"Algo correu mal ao tentar actualizar as informações de perfil";
