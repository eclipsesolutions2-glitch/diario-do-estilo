...
export async function createArticleAction(formData: FormData,): Promise<ApiResponse<boolean>> {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const msg = json ?? "Algo correu mal ao tentar criar um artigo";
            return ApiResponseBuilder.error(msg);
        }
        return ApiResponseBuilder.success(true);
    } catch (error) {
        const errorMessage = "Falha ao criar um artigo";
