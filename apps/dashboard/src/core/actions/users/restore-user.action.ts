...
export async function restoreUserAction({
  userId,
}: RestoreUserActionParams): Promise<ApiResponse<RestoreUserActionResponse>> {
  ...
  try {
    ...
    const response = await fetch(
      `${NEXT_PUBLIC_API_URL}/api/v1/auth/users/${userId}/restore`,
      {
        method: "POST",
        headers: {
    ...
    const json = (await response
      .json()
      .catch(() => null)) as RestoreUserActionResponse;
    if (!response.ok) {
      const msg =
        json.message ??
        "Algo correu mal ao tentar reativar a conta de" + userId;
