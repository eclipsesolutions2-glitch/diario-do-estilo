...
import { Overview } from "@/core/schemas/overview";
import { env } from "@/lib/env";
import { ApiResponse, ApiResponseBuilder } from "@workspace/ui/lib/mappers/api-response-builder.mapper";
import { cookies } from "next/headers";

export async function overviewDashboardAction(): Promise<ApiResponse<Overview>> {
    ...
    try {
        ...
        const response = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/admin/dashboard/overview`, {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
            const msg = "Algo correu mal ao tentar buscar dados de overiview";
            return ApiResponseBuilder.error(msg);
        }
        return ApiResponseBuilder.success(json);
    } catch (error) {
        const errorMessage = "Falha ao buscar dados de overiview";
