...
export async function findManyUnReadNotificationAction({ page = 1 }: FindManyUnReadNotificationParams): Promise<ApiResponse<Notification[]>> {
    ...
    try {
        ...
        const response = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/auth/notifications/unread?page=${page}`, {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${token.value}`
            }
        });
        const json = await response.json().catch(() => null) as FindManyUnReadNotificationResponse;
