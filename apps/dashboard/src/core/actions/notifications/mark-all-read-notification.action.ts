...
export async function markAllReadNotificationAction(): Promise<
  ApiResponse<MarkAllReadNotificationResponse>
> {
  ...
  try {
    ...
    const response = await fetch(
      `${NEXT_PUBLIC_API_URL}/api/v1/auth/notifications/read-all`,
      {
        method: "PATCH",
        headers: {
    ...
    const json = (await response
      .json()
      .catch(() => null)) as MarkAllReadNotificationResponse;
